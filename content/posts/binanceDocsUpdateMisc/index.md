...
API 설계는 대개 합리적이었지만 굳이 흠잡자면, 거래소 자체적인 상업적인 요소나 정책으로 인한 몇몇 불편함은 있었다. 가령, Isolated Margin Pair를 회원 등급에 따라 한 번에 운용할 수 있는 자산군을 작게는 10개 까지만 지원했다. 결국 자산군을 켜고 끄는 API Endpoint도 존재하고 문서에도 반영되어 있었다.

정리하자면,

1. 예전 Binance 문서는 UI 측면에서(긴 세로 스크롤) 개선할 여지가 있어 보였지만, 치명적인 문제는 없었다.
2. API 아키텍칭의 측면에서는 정말 세심한 부분까지 합리적으로 잘 설계했다.
3. 문서나 API 아키텍칭 외적으로 거래소 정책으로 인해 불편한 지점이 문서와 API 아키텍칭에도 그대로 잘 명세되었다.(?)
