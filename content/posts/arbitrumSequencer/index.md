...
Arbitrum의 Transaction Type은 아래와 같이 기존 geth에서 정의된 것들에 더해, Arbitrum Nitro에서 특별히 정의한 유형이 더 추가되었다. 당연한 얘기지만 각각의 Transaction Type마다 역할과 의미가 다르다. `ParseL2Transaction()` 함수 및 `parseL2Message()` 함수에서, 해당 트랜잭션의 유형에 따라 `switch-case` 문을 통해 Transaction 파싱이 적절하게 이루어져 반환하는 것을 알 수 있다.
